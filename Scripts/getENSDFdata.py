...
import urllib.error #import urllib2
import re
import sys
import chemistry
import socket
from urllib.request import Request #from urllib2 import Request
from urllib.error import URLError #
from bs4 import BeautifulSoup #from sgmllib import SGMLParser

class URLLister(BeautifulSoup): #class URLLister(SGMLParser):
  def reset(self):
    BeautifulSoup.reset(self) #SGMLParser.reset(self)
    self.urls = []
...
def getPage(url, ele, A):
    ...
    page = ''
    try:
        page = urllib.request.urlopen(req,timeout=3) #page = urllib2.urlopen(req,timeout=3)
    except socket.timeout:
        print ("ERROR: TIMEOUT") # print "ERROR: TIMEOUT" = without parentheses 
        print (url) # url = without parentheses 
...
def getURL(ele, A):
    ...
    for url_end in url_ends:        
        ...
        print(('Retrieving ENSDF data from:\t',url)) # print 'Retrieving ENSDF data from:\t',url 
        req = Request(url)
        page = ''
        try:
            page = urllib.request.urlopen(req,timeout=3) #page = urllib2.urlopen(req,timeout=3)
