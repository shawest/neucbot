...
def generateAlphaFileName(ele, A): #возвращает строку = имя альфа списка
    outdir = './AlphaLists/'
...
def readTargetMaterial(fname):
    ...
    mat_comp = [] #создание пустого списка
    tokens = [line.split() for line in f.readlines()] #вложенный список или двумерный массив
    #readlines() выводит список из строк, split() разделяет строку по пробелам
    for line in tokens: #обращение к каждой строчке последовательно
        if len(line) < 3: #если кол-во слов в строке < 3...
            continue
        if line[0][0] == '#': #если кто-то пишет везде комментарии....
            continue
        ele = line[0].lower() #материал
        A = int(line[1]) #массовое число. если == 0, то элемент часто встречается в природе
        frac = float(line[2]) # % содержание

        if A == 0:
            natIso_list = gni.findIsotopes(ele).split() #возвращается список
            for A_i in natIso_list:
                #abund = float(isoabund.findAbundance(str(A_i) + ele.capitalize())) - было
                #list_to_string= ','.join(isoabund.findAbundance(str(A_i) + ele.capitalize()))
    ...
    #Normalize
    #norm = 100 #был 0 ...
#    for mat in mat_comp: - убрала, так как программа не хочет делить на 0
#        norm += mat.frac
    for mat in mat_comp:
    	norm = 100
    	mat.frac = mat.frac/norm
        #mat.frac /= norm - было
        # изменила / на //, так как в python2 оператор / выполняет целочисленное деление
        # в python3  оператор целочисленного деления - //
    return mat_comp
...
def getIsotopeDifferentialNSpec(e_a, ele, A):
    ...
    if not os.path.exists(fname):
        if constants.run_talys:
            ...
            print("Warning, no (alpha,n) data found for E_a =", e_a, "MeV on target", target,
                  "...skipping. Consider running with the -d or -t options", file=constants.ofile)
            return {}
...
def run_alpha(alpha_list, mat_comp, e_alpha_step):
    ...
    for [e_a, intensity] in alpha_ene_cdf:
        ...
        if counter % (int(len(alpha_ene_cdf) / 100)) == 0:
            sys.stdout.write('\r')
            sys.stdout.write(
                "[%-100s] %d%%" % ('=' * int(counter * 100 / len(alpha_ene_cdf)), 100 * counter / len(alpha_ene_cdf)))
            sys.stdout.flush()

        stopping_power = 0 #- деление на ноль???
        #stopping_power = 0.1 #немного изменила значение....
        if stopping_power == 0:
        ...
        for mat in mat_comp:
            ...
            if e_a - e_alpha_step < 0:
                delta_ea = e_a
            prefactors = (intensity / 100.) * mat_term * delta_ea / stopping_power #- ошибка float division by zero
    ...
    sys.stdout.write('\r')
    sys.stdout.write(
        "[%-100s] %d%%" % ('=' * int((counter * 100) / len(alpha_ene_cdf)), 100 * (counter + 1) / len(alpha_ene_cdf)))
    sys.stdout.flush()
...
#    if not mat_comp or not alpha_list:
#        if not alpha_list: print('No alpha list or chain specified', file=sys.stdout)
#        if not mat_comp: print('No target material specified', file=sys.stdout)
#        print('', file=sys.stdout)
#        help_message()
#        return 0
        
#    if len(mat_comp) == 0 or len(alpha_list) == 0:
